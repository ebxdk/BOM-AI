        ...
        if profile_data:
            ...
            if profile.get('assessment_completed', False):
                ...
                session['result'] = {
                    ...
                    'rank': profile.get('rank', 'No rank available')  # Ensure rank is included
                }
                session.modified = True
...
def get_gpt4_capacity_message(username, energy_score, purpose_score, connection_score, total_score, user_state, recommendations):
    ...
    # Extract just the names of the tools for the final prompt text
    tool_names = [rec['tool'] for rec in recommendations]

    # Generate the final message prompt for the GPT-4 LLM
    prompt = f"""
    ...
    # Call the LLM with the prompt and get the response directly
    response = llm(prompt)
        ...
        if profile_data:
            ...
            if profile.get('assessment_completed', False):
                session['result'] = {
                    ...
                    'connection_score': profile['baseline_scores'].get('connection', 0),
                    'user_state': profile.get('user_state', 'Unknown'),
                    'total_score': profile.get('total_score', 0),  # Include total_score
                    'recommendations': profile.get('recommendations', []),
                    'recommendation_message': profile.get('recommendation_message', 'No recommendation available'),
...
def create_account():
    if request.method == 'POST':
        ...
        if profile_data:
            ...
            profile = {
                'username': username,
                'assessment_completed': False,  # Add this line
                'baseline_scores': {
                    'energy': 0,
                    'purpose': 0,
                    'connection': 0
...
# Endpoint to retrieve ranked leaderboard
...
def retake_assessment():
    ...
    if profile_data:
        profile = json.loads(profile_data)
        # Set assessment_completed to False
        profile['assessment_completed'] = False

        # Optionally reset other fields if you want to clear previous data
        ...
        # Redirect back to index, which will now show the questionnaire
        return redirect(url_for('index'))

    else:
        # If no profile data is found, redirect to login
        return redirect(url_for('login'))
